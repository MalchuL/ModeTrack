...
const CodeBlock = ({ inline, className, children, ...props }: CodeBlockProps) =>
  inline ? (
    ...
    <code
      className={`block rounded bg-muted p-3 text-sm font-mono text-foreground ${className ?? ""}`}
      {...props}
    >
      {children}
    </code>
...
const markdownComponents: Components = {
  ...
  p: ({ node, ...props }) => <p className="leading-relaxed text-muted-foreground mb-4" {...props} />,
  ul: ({ node, ...props }) => (
